...
// STEP: 2 READING A LEVEL -> PLAYER LEVEL (The following class stores a level object. Its argument should be the string that defines the level.)
class Level {
    constructor(plan) {
...
// TRACKING THE STATE => We’ll use a State class to track the state of a running game.
class State {
...
// We change the scroll position by manipulating that element’s scrollLeft and scrollTop properties when the player is too close to the edge.
...
// STEP: 6 ACTOR UPDATES
// Actor object's update methods take as arguments the time step, the state object, and a keys object. The one for the Lava actor type ignores the keys object.
